...
export const SalesChart: React.FC<SalesChartProps> = ({ data }) => {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    const department = payload[0].payload.department?.split(':')[0] || 'Unknown';
    const value = payload[0].value || 0;
    const profit = payload[1].value || 0;

    return (
      <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
        <p className="text-gray-900 font-medium mb-1">{department}</p>
        <p className="text-gray-800">
          <span className="font-medium">Value:</span> £{value.toLocaleString()}
        </p>
        <p className="text-gray-800">
          <span className="font-medium">Profit:</span> £{profit.toLocaleString()}
        </p>
      </div>
    );
  };
