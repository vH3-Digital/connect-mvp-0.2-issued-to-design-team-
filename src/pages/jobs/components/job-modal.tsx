...
import { IJobModal } from "../../../interfaces/interfaces";

export const JobModal: React.FC<IJobModal> = ({ isOpen, onClose, job }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 mb-2 sm:mb-0">
          <Tag className="w-5 h-5 min-w-5 text-primary" />
        ...
        <span
          className={`px-3 py-1 rounded-full text-sm whitespace-nowrap ${
            job.Status.toLowerCase() === "completed"
              ...
              : job.Status.toLowerCase() === "completed with issues"
      ...
      {/* Contact Info */}
      <div className="bg-bgSecondary rounded-lg p-4 space-y-3">
        ...
        <div className="sm:pl-7 space-y-2">
          ...
          <div className="flex items-center gap-2">
            <MapPin className="w-4 h-4 min-w-4 text-gray-400" />
            <p>{job.Location}</p>
          </div>
              ...
              <div key={index} className="flex justify-between items-center">
                ...
                {assistant.Start && assistant.End && (
                  <span className="text-sm text-gray-400">
                    {format(new Date(assistant.Start), "HH:mm")} -{" "}
                    {format(new Date(assistant.End), "HH:mm")}
                  </span>
                )}
              </div>
            ))}
